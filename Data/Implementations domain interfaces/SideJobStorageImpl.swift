...
class SideJobManagerImpl: SideJobManagerRepository, MemoryTrackable{
    static let shared = SideJobManagerImpl()
    
    private let entity = CoreDataEntities.sideJobEntity.name
    
    private lazy var persistentContainer: NSPersistentContainer = {
        let contanier = NSPersistentContainer(name: "EarnLog")
        contanier.loadPersistentStores { desc, error in
            if let error {
    ...
    func loadActiveJobs() async throws -> [SideJob] {
        try await withCheckedThrowingContinuation { continuation in
            persistentContainer.performBackgroundTask { context in
                do {
                    let fetchRequest: NSFetchRequest<SideJobEntity> = SideJobEntity.fetchRequest()
                    fetchRequest.predicate = NSPredicate(format: "isActive == true")
                    let result = try context.fetch(fetchRequest)
                    continuation.resume(returning: result.map { $0.toSideJob() })
                } catch {
    ...
    func getAllJobs() async throws -> [SideJob] {
        let fetchRequest: NSFetchRequest<SideJobEntity> = SideJobEntity.fetchRequest()
        let fetchResult = try await persistentContainer.performBackgroundTask { context in
            try context.fetch(fetchRequest)
        }
        if fetchResult.isEmpty {
            let excampleItems = createExampleJobsIfNeeded()
            return excampleItems
        }
        return fetchResult.map { $0.toSideJob()}
    }
    
    func saveNewJob(job: SideJob) async throws {
        try await persistentContainer.performBackgroundTask { context in
            let entity = SideJobEntity(from: job, context: context)
